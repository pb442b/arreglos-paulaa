...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < MAX_ALUMNOS; i++) {
        ...
        fgets(nombresAlumnos[i], NOMBRE_LEN, stdin);
        nombresAlumnos[i][strcspn(nombresAlumnos[i], "\n")] = '\0'; // Eliminar salto de línea
    }

    // Lectura de nombres de asignaturas
    for (int j = 0; j < MAX_ASIGNATURAS; j++) {
        printf("Ingrese el nombre de la asignatura %d: ", j + 1);
        fgets(nombresAsignaturas[j], NOMBRE_LEN, stdin);
        nombresAsignaturas[j][strcspn(nombresAsignaturas[j], "\n")] = '\0'; // Eliminar salto de línea
    }

    // Lectura de calificaciones de los alumnos
    ...
    // Cálculo y presentación de resultados por estudiante
    for (int estudiante = 0; estudiante < MAX_ALUMNOS; estudiante++) {
    ...
    // Cálculo y presentación de resultados por asignatura
    for (int asignatura = 0; asignatura < MAX_ASIGNATURAS; asignatura++) {
        promedioEstudiante = 0;
